...
function ArticleSidebarItem(props) {
  const {title, author, time, slug} = props;
  
  return (
    <article className="related-post__card">
      <a href={`/blog/${slug}`} className="related-post__title">{title}</a>
      <div className="related-post__info">
        <a className="related-post__author" href="#">{author}</a>
        <p className="related-post__date">
          ...
          {getCreatedTime(new Date(time),new Date())}</p>
      </div>
    </article>
  );
}
